...
def train_and_test_svm(train, train_n, test, class_labels, stochastic=False, path_model="./", path_results="./"):
    ...
    for scorer in scoring:
        ...
        print("Optimal number of features: ", best_model.n_features_)
        plot_rfecv_results(best_model.grid_scores_, reg_strengths[best_model_index], scorer, show=False, save=True, title_suffix=train_n ,path=path_results + train_n)
        ...
        selected_features = get_selected_features(X_trn, best_model.support_)
...
def replot(train, train_n, test, class_labels, path_model="./", path_results="./"):
    """
    This method replots results from the models generated by the train_and_test_svm method.
    Takes the same inputs as train_and_test_svm with the exception of:
        stochastic parameter
        path_model is now the path of the saved model to be loaded
    """
    X_trn = train.iloc[:, :-1]
    y_trn = train.iloc[:, -1]
    X_tst = test.iloc[:, :-1]
    ...
    reg_strengths = np.arange(0.001, 0.4, 0.02).round(decimals=3)
    scoring = ["F1-Score"]
    rfecv_lists = pickle.load(open(path_model, "rb"))
    max_test_scores = []
    for rfecv_list, scorer in zip(rfecv_lists, scoring):
        max_train_score = []
        max_test_score = []
        for rfecv, c in zip(rfecv_list, reg_strengths):
            max_train_score += [rfecv.score(X_trn, y_trn)]
            max_test_score += [rfecv.score(X_tst, y_tst)]
        max_test_scores += [max_test_score]
        ...
        pickle.dump(selected_features, open(path_results + train_n + "_usefulFeatures_model_optimisation_strat_" + scorer + ".obj", "wb"))
    # making plot combining all scorers vs reg on a single figure
    plot_multiscore_comp(reg_strengths, max_test_scores, scoring, show=False, save=True, title_suffix=train_n, path=path_results + train_n + '_')
